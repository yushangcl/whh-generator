...
public class DateUtil {
    ...
    public static String formatTimestampSimple(Date date) {
        return format(date, YYMMDD_SIMPLE);
    }
    ...
    public static Date getAfterManyDays(Date date, Integer months, String fmt) throws Exception {
        if (null == fmt) {
            fmt = "yyyy/MM/dd";
        }
        if (null == date) {
...
//        System.out.println(DateUtil.format(getMonthFirstDay(new Date(), 0),"yyyy-MM-dd HH:mm:ss"));
        System.out.println(DateUtil.formatTimestampSimple(DateUtil.getBeforeDate(1)));
    }
